"""
Implementation of the GJR-GARCH(1,1) model.

Model:
    r_t = epsilon_t
    I_t = 1 if r_t < 0 else 0
...
@njit
def gjr_garch_recursion(residuals, omega, alpha, beta, gamma):
    T = len(residuals)
    var = np.empty(T)
    var[0] = np.var(residuals) # Try unconditional variance

    for t in range(1, T):
        neg_shock = 1 if residuals[t - 1] < 0 else 0
        var[t] = omega + (alpha + gamma * neg_shock) * residuals[t - 1]**2 + beta * var[t - 1]
...
class GJRGARCHModel(VolatilityModel):
    ...
    def log_likelihood(self, params):
        ...
        var = gjr_garch_recursion(self.residuals, omega, alpha, beta, gamma)
    ...
    def fit(self, initial_guess=None): # Try different initial guesses
        if self.dist == 'normal':
            if initial_guess is None:
                initial_guess = [1e-6, 0.05, 0.9, 0]
            bounds = [(1e-12, None), (0, 1), (0, 1), (-1, 1)]
            method = 'L-BFGS-B'
        elif self.dist == 't':
            if initial_guess is None:
                initial_guess = [1e-6, 0.05, 0.9, 0, 8]
            bounds = [(1e-12, None), (0, 1), (0, 1), (-1, 1), (2.5, 100)]
            method = 'Nelder-Mead'
        elif self.dist == 'ged':
            if initial_guess is None:
                initial_guess = [1e-6, 0.05, 0.9, 0]
        ...
        if self.dist == 'normal':
            ...
            omega, alpha, beta, gamma, _ = self.params
        elif self.dist == 'ged':
            omega, alpha, beta, gamma = self.params

        self.conditional_variance = gjr_garch_recursion(self.residuals, omega, alpha, beta, gamma)
    ...
    def forecast(self, horizon=1):
        ...
        last_neg_shock = 1 if last_residual < 0 else 0

        forecasts = np.zeros(horizon)
        forecasts[0] = omega + (alpha + gamma * last_neg_shock) * last_residual**2 + beta * last_var

        # For multiday forecasting
        for t in range(1, horizon):
            forecasts[t] = omega + (alpha + 0.5 * gamma + beta) * forecasts[t - 1]
